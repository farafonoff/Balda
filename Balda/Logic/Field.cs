...
namespace Balda.NET.Logic
{
    ...
    public class Field : INotifyPropertyChanged
    {
        ...
        {
            if (iword == null) iword = new string('A', FieldSize);
        ...
        void setupPlayers()
        {
            ...
            _playerz = new Player[2] {
                ...
                us.PlayersCount.count==2?new Player(PlayerType.Human,us.User2name):new Player(PlayerType.AI,us.PhoneLevel.Name+" "+AppResources.tPhone)
            };
        ...
        int aiWordDisplayTime { get { if (demo) return 1500; return 8000; } }
                ...
                case LetterState.Empty:
                    if (state == FieldState.AddingLetter)
                    {
                        if (lPosition.around().Where(let => this[let].state != LetterState.Empty).Count() == 0)
                            throw new GameException(AppResources.fcfNearTitle, AppResources.fcfNearText);
        ...
        internal void loadState(Dictionary<string,object> settings)
        {
                ...
                int il = 0;
                if (lcells.Count != FieldSize * FieldSize) throw new Exception("field size mismatch");
                for (int i = 0; i < FieldSize; ++i)
                {
                    for (int j = 0; j < FieldSize; ++j)
                    {
